...
def test_sniff_image_format_detects_raw_from_real_sample_extension() -> None:
    with SAMPLE_CR3.open("rb") as image_file:
        assert sniff_image_format(image_file.read(), SAMPLE_CR3.name) == ImageFormat.CR3


@requires_enabled_integration_suite
@requires_sample_images
def test_sniff_image_format_detects_dng_from_real_sample_extension() -> None:
    with SAMPLE_DNG.open("rb") as image_file:
        assert sniff_image_format(image_file.read(), SAMPLE_DNG.name) == ImageFormat.DNG


@requires_rawpy_decoder
@requires_enabled_integration_suite
def test_sample_cr3_converts_with_rawpy_when_supported() -> None:
    with SAMPLE_CR3.open("rb") as image_file:
        try:
...
@requires_rawpy_decoder
@requires_enabled_integration_suite
def test_sample_dng_converts_with_rawpy_when_supported() -> None:
    with SAMPLE_DNG.open("rb") as image_file:
        try:
            result = make_service().process_and_extract_metadata(
                image_file=image_file,
