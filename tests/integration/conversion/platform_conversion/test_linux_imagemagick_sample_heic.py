...
requires_linux = pytest.mark.skipif(
    ...
    reason="Linux ImageMagick HEIC conversion test runs only on Linux.",
)
...
def test_sample_heic_converts_with_linux_imagemagick() -> None:
    if not LinuxImageMagickImageConverter(timeout_seconds=20).can_convert("heic"):
