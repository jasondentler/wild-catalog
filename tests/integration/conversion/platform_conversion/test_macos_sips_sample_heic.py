...
requires_macos = pytest.mark.skipif(
    ...
    reason="macOS sips HEIC conversion test runs only on macOS.",
)
...
def test_sample_heic_converts_with_macos_sips() -> None:
    if not MacOSSipsImageConverter(timeout_seconds=20).can_convert("heic"):
        pytest.skip("macOS sips utility is not available for HEIC conversion.")
    ...
    with SAMPLE_HEIC.open("rb") as image_file:
        try:
            ...
            pytest.skip(f"macOS sips could not convert sample HEIC: {exc}")
