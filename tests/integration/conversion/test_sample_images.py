...
requires_rawpy_decoder = pytest.mark.skipif(
    not SAMPLE_CR3.exists(),
    reason="Sample CR3 image is not available.",
)
...
def make_service(
    *,
    ...
    platform_image_converter: str = "none",
...
@requires_enabled_integration_suite
@requires_sample_images
def test_all_sample_image_formats_are_covered() -> None:
    suffixes = {path.suffix.lower() for path in SAMPLE_IMAGES.iterdir()}

    assert {".jpg", ".heic", ".cr3"}.issubset(suffixes)
